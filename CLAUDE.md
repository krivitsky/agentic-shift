...
- **The five shifts render in two labelled groups** — *technical* (01–02, amber, sourced to the Agentic Engineer's Field Guide) and *organizational* (03–05, teal, sourced to 10X ORG). Grouping is structure, not copy: `accent`, `count` and `href` live in `en.json`'s `groups` array only and are read from there for every language; each translation supplies just the localized `label` + `link` text, so the grouping can't drift. The source links live in the group headers — that's why `systemNote` is a single paragraph now.
